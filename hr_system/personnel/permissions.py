...
class CanReadEntireOrganization(RoleBasePermission):
    ...
    def has_permission(self, request, view):
        ...
        if request.user.is_superuser: return True # Superuser overrides role check
...
class CanReadOwnDepartment(RoleBasePermission):
    ...
    def has_permission(self, request, view):
        ...
        profile = self.get_user_profile(request.user)
        # TODO: Replace MockUserRole with actual UserRole from models
        return profile and profile.role == MockUserRole.ROLE_2 and profile.division_assignment is not None
...
class CanEditOwnManagement(RoleBasePermission):
    ...
    def has_permission(self, request, view):
        ...
        profile = self.get_user_profile(request.user)
        # TODO: Replace MockUserRole with actual UserRole from models
        if not (profile and profile.role == MockUserRole.ROLE_3 and profile.division_assignment is not None):
            return False
