...
class DivisionViewSet(viewsets.ModelViewSet):
    ...
    def status_summary(self, request):
        ...
        for div in queryset:
            div.summary_data = {
                'id': div.id, 'name': div.name, 'division_type': div.division_type,
                'indicator': 'UNKNOWN', 'updated_children': 0, 'total_children': 0, 'children': []
            }
            if div.parent_division_id in division_map:
                parent = division_map[div.parent_division_id]
                if hasattr(parent, 'summary_data'):
        ...
        def calculate_indicators(node):
            ...
            total_children, updated_children = 0, 0
...
class EmployeeViewSet(viewsets.ModelViewSet):
    ...
    def terminate(self, request, pk=None):
        ...
        employee.save()
        return Response({'status': f'Employee {employee.full_name} has been terminated.'}, status=status.HTTP_200_OK)
    ...
    def transfer(self, request, pk=None):
        ...
        if profile.role == UserRole.ROLE_5:
            ...
            if not scope_checker.has_object_permission(request, self, employee.division) or \
               not scope_checker.has_object_permission(request, self, new_division):
                return Response(
...
class EmployeeStatusLogViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [permissions.IsAuthenticated] # Basic permission, anyone can log their own status perhaps? Refine later.

    def perform_create(self, serializer):
        # The serializer's create method already sets created_by
        serializer.save()
