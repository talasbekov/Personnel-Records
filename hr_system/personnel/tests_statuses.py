...
class StatusLogicTest(APITestCase):
    def setUp(self):
        ...
        # Create and authenticate a user with sufficient role for status log creation
    ...
    def test_auto_revert_to_default_status(self):
        ...
        self.assertEqual(
            ...
            EmployeeStatusType.ON_DUTY_SCHEDULED,  # default 'В строю'
    ...
    def test_no_status_logs_returns_default(self):
        ...
        """
        status_now = self.employee.get_current_status()
        self.assertEqual(status_now, EmployeeStatusType.ON_DUTY_SCHEDULED)
    ...
    def test_conflicting_status_overlap_is_prevented(self):
        ...
        existing one is prevented by the serializer / API validation.
        """
        ...
        # Attempt to create a new conflicting status: Business Trip overlapping
        ...
        self.assertIn("conflicts with an existing status", response.data.get("non_field_errors", [""])[0])
    ...
    def test_allowed_status_overlap_is_permitted(self):
        ...
        # New status: On Leave overlapping
        url = "/api/personnel/status-logs/"
        data = {
            "employee_id": self.employee.id,
            "status": EmployeeStatusType.ON_LEAVE,
